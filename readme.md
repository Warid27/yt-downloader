...
1. Place all three files in a folder:
...
3. When prompted, enter the download root directory (or accept default = script directory).
...
## 🔄 Auto-Update (Maintenance-Free)

Both scripts keep themselves current without manual work:

- **yt-dlp**: On every run, calls `yt-dlp --update-to nightly`. YouTube breaks extractors weekly, so this matters. If that fails, falls back to direct download from GitHub releases.
- **ffmpeg**: If not found in `PATH` or `./ffmpeg-master-latest-win64-gpl-shared/bin/`, downloads the portable build from gyan.dev and extracts to script dir.

You never have to redownload a binary again.

## 📁 Folder Structure

```
E:\yt-downloader\                       (or any folder you choose)
...
├── yt-dlp.exe                          Auto-updated
├── ffmpeg-master-latest-win64-gpl-shared\   Auto-extracted
│   └── bin\ffmpeg.exe
├── video.txt                           Your video URL list
├── audio.txt                           Your audio URL list
└── results\
...
- `results\error_log.txt` — only failed URLs (use for retry)

## ⚠️ Troubleshooting

### "yt-dlp is not installed or not in PATH"
Should not happen — script auto-downloads. If it does, check internet connectivity. Manual fallback: <https://github.com/yt-dlp/yt-dlp/releases/latest>

### "ERROR: Unable to extract audio" / "ffmpeg not found"
Script should have downloaded ffmpeg. If it failed, manually grab the essentials build: <https://www.gyan.dev/ffmpeg/builds/>

### "n challenge solving failed" / SABR streaming warnings
YouTube changes its anti-bot measures. The script's auto-update to `nightly` build usually fixes this within hours. If not, see `DEV.md` for PO Token workaround.
...
yt-dlp supports `# FORMAT=` comments in the URL file. The script doesn't parse these — use the `FORMAT` config var.
